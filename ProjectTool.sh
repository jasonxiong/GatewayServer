...
Usage()
{
	echo "Usage: ./ProjectTool.sh -I <SERVERIP> <ACCOUNT_PORT> <ZONE_PORT> <REGAUTH_PORT> <WORLDID> <ZONEID>"
	echo "                        --Init Server Project"
	echo "       ./ProjectTool.sh -B"
	echo "                        --Build Server Project and Make Release Package"
	echo "		 ./ProjectTool.sh -P"
	echo "						  --Pack the Release Package"
...
Init_Server()
{
	...
	svn switch --ignore-ancestry svn://192.168.78.129/qmonster/tools/ConvertXml/xml xml
	cd ../../../Build/

	#svn switch proto的文件到源代码目录
	cd ../103_Protocol/Conf/
	svn switch --ignore-ancestry svn://192.168.78.129/qmonster/tools/GameProto proto
	cd ../../Build/

	#svn switch 最新配置到RuntimeEnv目录
	cd RuntimeEnv/
	svn switch --ignore-ancestry svn://192.168.78.129/qmonster/tools/ConvertXml/bin/server Common
	cd ../
	...
	#设置所有配置文件中的服务器通信BUS
	perl -i -p -e 's/'#WORLDID#'/'$5'/g' ~/RuntimeEnv/Cluster/ClusterServer/conf/GameServer.tcm ~/RuntimeEnv/Zone/ZoneServer/conf/GameServer.tcm ~/RuntimeEnv/World/AccountServer/conf/GameServer.tcm ~/RuntimeEnv/World/RoleDBServer/conf/GameServer.tcm ~/RuntimeEnv/World/WorldServer/conf/GameServer.tcm ~/RuntimeEnv/World/NameDBServer/conf/GameServer.tcm

	perl -i -p -e 's/'#ZONEID#'/'$6'/g' ~/RuntimeEnv/Zone/ZoneServer/conf/GameServer.tcm ~/RuntimeEnv/World/WorldServer/conf/GameServer.tcm

	perl -i -p -e 's/'#SERVERIP#'/'$1'/g' ~/RuntimeEnv/Cluster/ClusterServer/conf/GameServer.tcm ~/RuntimeEnv/Zone/ZoneServer/conf/GameServer.tcm ~/RuntimeEnv/Global/RegAuthServer/conf/GameServer.tcm ~/RuntimeEnv/Global/AccountDBServer/conf/GameServer.tcm ~/RuntimeEnv/World/AccountServer/conf/GameServer.tcm ~/RuntimeEnv/World/RoleDBServer/conf/GameServer.tcm ~/RuntimeEnv/World/WorldServer/conf/GameServer.tcm ~/RuntimeEnv/World/NameDBServer/conf/GameServer.tcm
...
Pack_Server()
{
	...
	if [ ! -e "release/bin/AccountServer.dbg" ] ||
		...
		[ ! -e "release/bin/RoleDBServer.dbg" ] ||
		[ ! -e "release/bin/ZoneServer.dbg" ] ||
		[ ! -e "release/bin/ClusterServer.dbg" ] ||
		[ ! -e "release/bin/RegAuthServer.dbg" ] ||
		[ ! -e "release/bin/AccountDBServer.dbg" ] ||
		[ ! -e "release/bin/NameDBServer.dbg" ]
	then
	...
	tar -zcvf Packages/RuntimeEnv.tar.gz RuntimeEnv/ --exclude LotusListener.tcm --exclude *svn* --exclude log --exclude conf --exclude TCP* --exclude ScriptFrameSvr* --exclude bills
