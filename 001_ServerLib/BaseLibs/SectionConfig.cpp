/************************************************************
  ...
  Others:
  Function List:
    1. CSectionConfig::CSectionConfig
       构造函数，初始化类的成员，读取配置文件
    2. CSectionConfig::~CSectionConfig
       析构函数，释放资源
    3. CSectionConfig::IsOpen
       返回配置文件读取状态
    4. CSectionConfig::GetItemValue字符串
       取指定的键值
    5. CSectionConfig::LocateSection
       定位section的开始地址和结束地址
    6. CSectionConfig::LocateKeyValue
       在指定的缓冲区范围中搜索Key，返回与Key匹配的值的开始地址和结束地址
    7. CSectionConfig::LocateStr
       在一个字符串中搜索另一个字符串
    8. CSectionConfig::MapToContent
       将Shadow中的地址映射到Content中
    9. CSectionConfig::MapToShadow
       将Content中的地址映射到Shadow中
    10.ToLower
       将字符串中的大写字母转换成小写字母
    18.CSectionConfig::OpenFile
       加载配置文件到内存
    19.CSectionConfig::CloseFile
       释放配置文件加载到内存后占用的资源
    20.CSectionConfig::SetItemValue字符串
       从内存缓冲区中找到KeyName，将值拷贝到指定的空间
    21.CSectionConfig::LocateKeyRange
       在指定的缓冲区范围中搜索Key，返回与Key匹配的值的开始地址和结束地址
    22.CSectionConfig::SetItemValue数字
    23.CSectionConfig::GetItemValue数字
***********************************************************/
